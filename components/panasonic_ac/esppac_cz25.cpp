...
climate::ClimateAction PanasonicACCZ25::determine_action_from_cnt_state_(uint8_t state) {
  ...
  // mode or current-vs-target temperature.
  switch (state) {
    case 0x00:
    case 0x04:
...
void PanasonicACCZ25::publish_cnt_telemetry_() {
  ...
  if (this->raw_status_packet_sensor_ != nullptr) {
    auto raw_packet = format_hex_pretty(this->rx_buffer_);
    this->raw_status_packet_sensor_->publish_state(raw_packet);
  }
  ...
  // The hand-warming test shows b21 follows the same intake-air thermistor path
  // very closely, often at +1 C. Keep the public name neutral until the exact
  // Panasonic processing relationship is identified.
